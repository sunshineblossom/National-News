$(document).ready(function () {
    ...
    // function checkUser() {
    //     //如果不是本人
    //     if($("#userId").val()!=$("#uid").val()){
    //
    //         //    将删除选项除去
    //         $(".log-comment").removeClass("col-lg-3");
    //
    //         $(".log-handup").removeClass("col-lg-3");
    //         $(".log-share").removeClass("col-lg-3");
    //         $(".log-delete").css("display","none");
    //         $(".log-comment").addClass("col-lg-4");
    //
    //         $(".log-handup").addClass("col-lg-4");
    //         $(".log-share").addClass("col-lg-4");
    //     }
    // }
    ...
    $("body").on("click", ".article", function () {
        ...
        hide_edit_content();
        //校验右侧标志位
        $(".focus-fans-module").css("display", "none");
        $(".btn-edit-article").css("display", "block");
        // 1.清空content内容
        $(".content").empty();
        // 2.添加文章列表需要使用ajax查询数据
        var data = {uid:$("#uid").val()};
        $.ajax({
            url: basePath + "/news/selectNewsByEditorId",
            type: "POST",
            dataType: "json",
            contentType: "application/json;charset=utf-8",
            cache: false,
            async: true,
            data:JSON.stringify(data) ,
            success: function (newsCustoms) {
                for (var i = 0; i < newsCustoms.length; i++) {
                    var publishTime = new Date(newsCustoms[i].publishTime)
                    $(".content").append("<div class=\"mylog\"><div class=\"log-head\"><a href=\"#\"><img class=\"log-user-img\" src=\"" + $(".user-img").attr("src") + "\"></a><div class=\"row log-username\"><a href=\"#\">" + $(".pf-username").text()+ "</a></div>" +
                        "<div class=\"row log-publishtime\"><span>" + (publishTime.getMonth()+1) + "月" + publishTime.getDate() + "日 " + publishTime.getHours() + ":" + publishTime.getMinutes() + "</span></div></div><div class=\"log-content\">" + newsCustoms[i].content + "</div>" +
                        "<div class=\"log-footer\"><div class=\"row\"><div class=\"col-lg-3 log-share\" data-toggle=\"modal\" data-target=\".share\"><a href=\"#\"><span class=\"glyphicon glyphicon-share-alt\"></span>&nbsp;转发<span class='shareNumber'>" + newsCustoms[i].shareNumber + "</span></a></div>" +
                        "<div class=\"col-lg-3 log-comment\"><a href=\"javascript:void(0);\"><span style=\"display: none;\" class=\"click-count\">0</span><span class=\"glyphicon glyphicon-comment\"></span>&nbsp;评论<span class='commentNumber'>" + newsCustoms[i].commentNumber + "</span></a></div><div class=\"col-lg-3 log-handup\"><span class=\"up-click-number\" style=\"display:none;\">0</span><a href=\"javascript:void(0);\"><span class=\"glyphicon glyphicon-hand-up\"></span>&nbsp;点赞<span class='upNumber'>" + newsCustoms[i].upNumber + "</span></a></div>" +
                        "<div class=\"col-lg-3 log-delete news-delete\"><a href=\"#\"></span>&nbsp;删除</a></div></div></div></div>");
                }
            }
           
        });
        // 	2.1只显示文章标题若文章太长则设置最长显示长度
        // 3.将右侧关注和粉丝按钮换成“我要发布”按钮
        if ($(".right-flag").val() == "0") {
            $(".right-nav").html($(".edit-module").html());
            $(".right-flag").val("1");
        }
    })
    ...
    $("body").on("click", ".btn-edit-article", function () {
        //隐藏主题区
        $(".content").css("display", "none");
        //隐藏右侧导航栏
        $(".right-nav").css("display", "none");
        // 显示编辑区
        $(".news-edit-module").css("display", "block");
        //加载所有分类
        $.ajax({
            ...
            success: function (categoryList) {
                ...
                if(categoryList!=null){
                    ...
                    for(var i=0;i<categoryList.length;i++){
                        $(".dropdown-menu").append('<li><a href="javascript:void(0);" class="'+categoryList[i].categoryId+'">'+categoryList[i].categoryName+'</a></li>');
                    }
                }
            }
        });
   ...
   $("body").on("click",".dropdown-menu>li>a",function(){
       var ele = $(this);
       $("#dropdownMenu2").html(ele.text()+'<input type="hidden" id="categoryId" value="'+ele.attr("class")+'"><span class="caret"></span>');
   })
